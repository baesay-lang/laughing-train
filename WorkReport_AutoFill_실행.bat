...
if not exist "%MAIN_SCRIPT%" (
    ...
    echo 이 배치 파일과 같은 폴더에 파이썬 스크립트를 두거나,
    echo 배치 파일 상단의 MAIN_SCRIPT 값을 실제 파일명으로 바꿔 주세요.
    pause
    exit /b 1
)

REM ---- 1. 실제 Python 찾기 (MS 스토어 가짜 별칭은 걸러냄) ----
REM  스토어 별칭은 실행 시 오류 코드를 반환하므로 실제로 실행해서 확인한다
set "PYTHON_CMD="
...
if not defined PYTHON_CMD (
    ...
    REM 설치 직후에는 이 창의 PATH가 갱신되지 않으므로 알려진 경로에서 직접 찾는다
